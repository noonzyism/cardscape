...
class UserModel {
  ...
  List<UserCardModel> cardThumbs;
  List<UserPackModel> packThumbs;
  ...
  UserModel.fromUserDocument(Map<String, dynamic> parsedJson) {
    ...
    cardThumbs = cardData.map((c) => UserCardModel.fromJson(new Map<String, dynamic>.from(c))).toList();
    var packData = parsedJson['packs'] as List;
    packThumbs = packData.map((p) => UserPackModel.fromJson(new Map<String, dynamic>.from(p))).toList();
